...
export default function Reservations() {
  ...
  const handleEditStatus = (id) => {
    const newStatus = prompt("Enter new status (Pending, Confirmed, Cancelled):");
    if (!newStatus) return toast.error("Status required!");
  ...
  const handleAdd = () => {
    ...
    const people = Number(prompt("People:"));
    const status = prompt("Status (Pending, Confirmed, Cancelled):");
    ...
    setReservations([
      ...reservations,
      { id: Date.now(), customer, date, time, people, status },
    ]);
        ...
        <main className="p-6 overflow-auto">
          ...
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-3xl font-extrabold bg-clip-text text-transparent
              bg-linear-to-r from-blue-600 to-green-500 flex items-center gap-3">
            ...
            <button
              ...
              bg-linear-to-r from-green-500 to-emerald-600 hover:scale-105 transition"
            >
              + ADD
            </button>
          </div>
            ...
            <table className="min-w-full">
              <thead className="bg-linear-to-r from-blue-600 to-purple-600 text-white">
                <tr>
                  ...
                  <th className="py-4 px-6">PEOPLE</th>
                ...
                {reservations.map((res, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 flex items-center gap-2">
                      <FaUserFriends /> {res.customer}
                    </td>
                    <td className="py-4 px-6">{res.date}</td>
                    <td className="py-4 px-6">{res.time}</td>
                    <td className="py-4 px-6">{res.people}</td>
                    ...
                    <td className="py-4 px-6 flex gap-2 justify-center">
                      <button
                        onClick={() => handleEditStatus(res.id)}
                        className="px-3 py-1 text-white bg-blue-500 rounded hover:scale-105"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDelete(res.id)}
                        className="px-3 py-1 text-white bg-red-500 rounded hover:scale-105"
                      >
                        Delete
                      </button>
                    </td>
